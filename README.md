...
- **投前评估** — 粘贴 JD（支持 BOSS直聘 自动解析），AI + WebSearch 分析公司背景、JD 风险、薪资换算、简历匹配，给出决策报告
- **面试记录** — 完整的面试历史，支持语音输入，AI 自动提取标签（面试官风格、核心考点、风险/正面信号）
- **评估与面试关联** — 投前评估可与面试记录双向关联，对比"预期"与"实际"
- **分析面板** — 跨记录模式发现：高频考点、被拒原因分布、TOP5 核心考点
- **公司画像** — 按公司聚合面试记录、AI 标签、平均评分、结果分布
- **AI 连接测试** — 设置页内置测试连接 + WebSearch 测试，实时显示 SDK 调用完整地址
...
- **Interview** — 面试记录（公司、岗位、日期、轮次、薪资、通勤、工作制度、备注、AI标签）
- **PreInterviewAnalysis** — 投前评估（JD原文、分析结果、决策建议、关联面试记录）
